...
// Count Down JS code 2nd SLides
  function countUp(el, target, duration, divide) {
    let start = 0;
    const step = target / (duration / 16);
    const timer = setInterval(() => {
      start += step;
      if (start >= target) { start = target; clearInterval(timer); }
      const display = divide ? (start / 10).toFixed(1) : Math.floor(start).toLocaleString();
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const duration = 2000;
        const divide = el.dataset.divide === 'true';
        countUp(el, target, duration, divide);
  ...
  // ============================================================
  // Consultation form — validation + GTM dataLayer push + thank-you
  // state, no page reload. (Task 2 requirement)
  // ============================================================
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var thankYouPhone = document.getElementById('thankYouPhone');
    ...
    form.addEventListener('submit', function (e) {
      ...
      var phoneValue = phoneInput.value.replace(/\D/g, ''); // strip spaces/dashes
      ...
      // ---- GTM dataLayer push — fires only on a successful submit ----
      ...
      // ---- Swap form for thank-you state, no page reload ----
      thankYouName.textContent = nameValue;
      thankYouPhone.textContent = '+91 ' + phoneValue;
      form.hidden = true;
